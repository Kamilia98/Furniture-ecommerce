...
const updateStoreConfig = asyncWrapper(async (req, res, next) => {
  ...
  await StoreSettings.findOneAndUpdate(
    {},
    { storeName, defaultCurrency, defaultLanguage },
    { upsert: true }
  );
  ...
  if (supportedCurrencies && Array.isArray(supportedCurrencies)) {
    for (const currencyData of supportedCurrencies) {
      if (currencyData.code) {
        ...
        await Currency.findByIdAndUpdate(currencyData._id, currencyData, {
          new: true,
        });
      } else {
    ...
    const receivedCurrencyCodes = supportedCurrencies
      .map((c) => c.code)
      .filter(Boolean);
  ...
  if (supportedLanguages && Array.isArray(supportedLanguages)) {
    for (const languageData of supportedLanguages) {
      if (languageData.code) {
        ...
        await Language.findByIdAndUpdate(languageData._id, languageData, {
          new: true,
        });
      } else {
    ...
    const receivedLanguageCodes = supportedLanguages
      .map((l) => l.code)
      .filter(Boolean);
  ...
  if (shippingMethods && Array.isArray(shippingMethods)) {
    for (const methodData of shippingMethods) {
      if (methodData._id) {
        await ShippingMethod.findByIdAndUpdate(methodData._id, methodData, {
          new: true,
        });
      } else {
